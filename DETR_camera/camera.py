...
video_capture = cv2.VideoCapture(10)               #USBカメラ入力
# 幅と高さを取得
...
out = cv2.VideoWriter('output.mp4', fmt, frame_rate, size)
...
while True:                    # USBカメラ入力の場合
    ...
    _, frame = video_capture.read() #OpenCV動画読み込み用の関数
    #色空間変換用　今回のプログラムではOpenCVにビデオを撮り込み画像への落とし込みにPillowsを用いている
    #OpenCVは色の順番がBGR、PillowsはRGBを前提としているため変換が必要
    frame_cvt = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)  #Opencv読み込み用の関数、カメラだとここでエラーを吐く(おそらくカメラ画像が取り込めてない)
    ...
    out.write(output_image) #出力画像をoutという変数に書き込み
