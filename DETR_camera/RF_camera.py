...
def crop_bounding_box(image, xmin, ymin, xmax, ymax):
    xmin = print(xmin)
    ymin = print(ymin)
    xmax = print(xmax)
    ymax = print(ymax)
...
def put_rect(cv2_img, prob, boxes):
    ...
    return output_image , xmin,xmax , ymin,ymax
...
while True:
    ...
    ymin=output_image[3]
    ymax=output_image[4]
    cutting_image=crop_bounding_box(output_image, xmin, ymin, xmax, ymax)
    ...
    cv2.imshow(" ",cutting_image) #'Video'はウィンドウ名、画像の出力コマンド
