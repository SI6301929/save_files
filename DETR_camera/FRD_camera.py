...
out = cv2.VideoWriter('/content/gdrive/My Drive/videobox/camera_result.mp4', fmt, frame_rate, size)
...
while True:
    ...
    keep = probas.max(-1).values > 0.95  #確率0.9以下をカット
    ...
    #display
    output_image = put_rect(color_frame, probas[keep], bboxes_scaled) #検出に使用した画像にリスケールした検出ボックスとその確率の記載
